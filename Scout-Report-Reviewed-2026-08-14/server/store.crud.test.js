...
function loadStoreWithMockDb(seed = {}) {
  ...
  async function runQuery(text, params = []) {
    ...
    if (sql.startsWith('INSERT INTO scout_reports')) {
      const [
        id, farmId, farmName, cropType, variety, isGreenhouse,
        reportDate, implementationWeek, implementationYear, weather,
        temperature, humidity, location, organisationId, growerName, scoutName,
        fieldName, fieldArea, fieldAreaUnit, growthStage, plantingDate,
        expectedHarvestDate, visitPurpose, scoutingPattern, visitStartedAt,
        visitEndedAt, masterObservations, canonicalPayload, notes, status, ownerId,
        createdAt, updatedAt,
      ] = params;
      ...
      state.reports.push({
        ...
        scouting_pattern: scoutingPattern,
        visit_started_at: visitStartedAt,
        visit_ended_at: visitEndedAt,
        master_observations: masterObservations ? JSON.parse(masterObservations) : {},
        canonical_payload: canonicalPayload ? JSON.parse(canonicalPayload) : null,
        notes,
        status,
        owner_id: ownerId,
