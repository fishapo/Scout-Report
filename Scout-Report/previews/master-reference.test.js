...
test('form provides database-backed Autofill from DB using authenticated reports', () => {
  const html = fs.readFileSync(path.join(__dirname, 'user-form.html'), 'utf8');
  assert.match(html, /id="autofill-db-btn"/);
  assert.match(html, /async function autofillFromDb\(\)/);
  assert.match(html, /fetchWithAuth\('\/api\/reports\?limit=1'/);
  assert.match(html, /masterObservations/);
  assert.match(html, /Values remain editable/);
});
